    ...
    <div class="p-6 max-w-screen-lg mx-auto">
      ...
      <div class="bg-gray-800 text-white p-6 rounded-lg shadow-md mb-10">
        <h3 class="text-xl font-bold mb-4 flex items-center">
          <i class="fas fa-pencil-alt mr-2"></i> {{ isEditMode ? '공지사항 수정' : '공지사항 생성' }}
        </h3>
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <!-- 제목 입력 -->
            <div class="relative">
              <label class="block text-sm font-medium text-gray-300 mb-1">제목</label>
              <input
                type="text"
                v-model="form.title"
            ...
            <!-- 작성자 입력 -->
            <div class="relative">
              <label class="block text-sm font-medium text-gray-300 mb-1">작성자</label>
              <input
                type="text"
                v-model="form.author"
          ...
          <div class="relative mt-4">
            <label class="block text-sm font-medium text-gray-300 mb-1">내용</label>
            <textarea
          ...
          <button
            type="submit"
            class="bg-teal-500 hover:bg-teal-400 text-white font-semibold py-2 px-4 rounded w-3/12"
          >
            <i class="fas fa-save mr-2"></i> {{ isEditMode ? '수정하기' : '공지사항 생성' }}
          </button>
        </form>
        ...
        <!-- 성공 메시지 -->
        <p v-if="Statusmessage" class="text-green-500 mt-4">{{ Statusmessage }}</p>
      </div>
    ...
    methods: {
      async fetchNotices() {
        try {
          ...
          // 최신순으로 정렬
            this.notices = response.data.data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        } catch (error) {
      ...
      async handleSubmit() {
        try {
          if (this.isEditMode) {
            // 수정 모드일 경우
            const response = await api.post(`/notice/update/${this.editingNoticeId}`, this.form);
            this.Statusmessage = response.data?.data?.message || '공지사항이 성공적으로 수정되었습니다.';
          } else {
            // 공지사항 생성 모드일 경우
            const response = await api.post('/notice', this.form);
            this.Statusmessage = response.data?.data?.message || '공지사항이 성공적으로 생성되었습니다.';
          }
  
          this.fetchNotices(); // 공지사항 목록 갱신
          this.resetForm(); // 폼 초기화
        } catch (error) {
          this.errorMessage = error.response?.data?.message || '공지사항 처리 중 오류가 발생했습니다.';
        }
      },
